...
.wrap input {
    ...
    padding: 0 5px;
    border-radius: 2px;
    box-shadow: 1px 2px 6px 0px #000000ad;
}
.head .back {
    padding: 5px;
    color: #545454;
    ...
    <div class="container-fluid gm_main">
        <div class="head">
            <a class="f_l" href="javascript:history.go(-1)"><span class="back">< {{__('mb.Back')}}</a>
            <span>{{ __('mb.Online_banking_to_pay') }}</span>
            <a class="f_r" href="javascript:history.go(-1)" style="visibility: hidden"><span class="back">< {{__('mb.Back')}}</a>
        </div>
                        ...
                        <div class="col-md-10">
                                <select name="account" class="form-control">
                                    @foreach ($bank_card_list as $item)
                                        <option value="{{$item->id}}">{{ \App\Models\Base::$BANK_TYPE[$item->bank_id] }} {{ $item->card_no }}</option>
                                    @endforeach
                                </select>
