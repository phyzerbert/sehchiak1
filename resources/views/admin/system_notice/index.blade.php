                 ...
                 <table class="table table-bordered table-hover text-center">
                     ...
                     @foreach($data as $item)
                         <tr>
                             ...
                             <td>
                                 @if ($item->on_line == 1)
                                     <a href="{{ route('system_notice.check', ['id' => $item->getKey(), 'status' => 0]) }}" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo trans('words.are_you_sure_offline') ?>')">{{ __('words.offline') }}</a>
                                 @elseif($item->on_line == 0)
                                     <a href="{{ route('system_notice.check', ['id' => $item->getKey(), 'status' => 1]) }}" class="btn btn-success btn-xs" onclick="return confirm('<?php echo trans('words.are_you_sure_online') ?>')">{{ __('words.online') }}</a>
                                 @endif
