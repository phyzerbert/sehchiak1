            ...
            <div class="panel-body">
                <form class="form-horizontal" id="form" action="{{ route('activity.store') }}" method="post">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label for="subtitle" class="col-sm-2 control-label">{{__('words.Activity_list_picture_1_photos')}}(En)</label>
                            <div class="col-sm-7">
                                <span class="btn btn-primary fileinput-button">
                                    <i class="glyphicon glyphicon-plus"></i>
                                    <span>{{__('words.Select_the_file')}}</span>
                                    <input id="fileupload_en" type="file" name="file">
                                </span>

                                <div id="progress_en" class="progress">
                                    <div class="progress-bar progress-bar-success"></div>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="subtitle" class="col-sm-2 control-label">{{__('words.Activity_list_picture_1_photos')}}(Malay)</label>
                            <div class="col-sm-7">
                                <span class="btn btn-primary fileinput-button">
                                    <i class="glyphicon glyphicon-plus"></i>
                                    <span>{{__('words.Select_the_file')}}</span>
                                    <input id="fileupload_malaya" type="file" name="file">
                                </span>

                                <div id="progress_malaya" class="progress">
                                    <div class="progress-bar progress-bar-success"></div>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="subtitle" class="col-sm-2 control-label">{{__('words.Activity_list_picture_1_photos')}}(China)</label>
                            <div class="col-sm-7">
                                <span class="btn btn-primary fileinput-button">
                                    <i class="glyphicon glyphicon-plus"></i>
                                    <span>{{__('words.Select_the_file')}}</span>
                                    <input id="fileupload_zh_cn" type="file" name="file">
                                </span>

                                <div id="progress_zh_cn" class="progress">
                                    <div class="progress-bar progress-bar-success"></div>
                                </div>
                            </div>
                        </div>
            ...
            $('#fileupload_en').fileupload({
                ...
                dataType: 'json',
                    ...
                    var html = '<div class="pull-left" style="position:relative;margin: 10px;">' +
                                ...
                                '<input type="hidden" name="title_img_en" value="'+img_path+'">' +
                                '</div>';
                    $('#files').append(html)
            ...
            $('#fileupload_malaya').fileupload({
                ...
                dataType: 'json',
                    ...
                    var html = '<div class="pull-left" style="position:relative;margin: 10px;">' +
                                ...
                                '</a>' +
                            '<a href="javascript:;" class="glyphicon glyphicon-remove" style="position: absolute;right: 0;top: 0;" onclick="removeDiv(this)"></a>'+
                                '<input type="hidden" name="title_img_malaya" value="'+img_path+'">' +
                                '</div>';
                    $('#files').append(html)
                ...
                progressall: function (e, data) {
                    var progress = parseInt(data.loaded / data.total * 100, 10);
                    $('#progress_malaya .progress-bar').css(
                        'width',
                        progress + '%'
                    );
                }
            }).prop('disabled', !$.support.fileInput).parent().addClass($.support.fileInput ? undefined : 'disabled');
            $('#fileupload_zh_cn').fileupload({
                url: url,
                dataType: 'json',
                done: function (e, data) {
                    var res = data.result;
                    ...
                    var html = '<div class="pull-left" style="position:relative;margin: 10px;">' +
                                ...
                                '</a>' +
                            '<a href="javascript:;" class="glyphicon glyphicon-remove" style="position: absolute;right: 0;top: 0;" onclick="removeDiv(this)"></a>'+
                                '<input type="hidden" name="title_img_zh_cn" value="'+img_path+'">' +
                                '</div>';
                    $('#files').append(html)
                ...
                progressall: function (e, data) {
                    var progress = parseInt(data.loaded / data.total * 100, 10);
                    $('#progress_zh_cn .progress-bar').css(
                        'width',
                        progress + '%'
                    );
                }
