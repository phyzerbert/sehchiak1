     ...
     <section class="content">
         <div class="panel panel-primary">
             <div class="panel-heading">
                 <h3 class="panel-title">{{__('words.transaction_history')}}</h3>
             </div>
             <div class="panel-body">
                 @include('admin.tran_history.filter')
                 ...
                 <table class="table table-bordered table-hover text-center">
                     ...
                     @foreach($all_histories as $item)
                        <tr>
                            ...
                            <td>{{ $item->game_id }}</td>
                            <td @if ($item->type == 'deposit') style="color:green;"  @else style="color:red;" @endif">{{ $item->request }}</td>
                            {{-- <td>
                     ...
                     <tfoot>
                        <tr>
                            <td colspan="3" style="color:green;font-weight:bold">Total Deposit : +{{$total_deposit}}</td>
                 ...
                 <div class="clearfix">
                    ...
                    <div class="pull-right" style="margin: 0;">
                        {!! $all_histories->appends(['api_type' => $api_type, 'playerName' => $playerName, 'start_at' => $start_at, 'end_at' => $end_at])->links() !!}
                    </div>
                </div>
             </div>
         </div>
