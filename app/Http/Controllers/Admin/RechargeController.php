...
class RechargeController extends Controller
{
    ...
    public function confirm(Request $request, $id)
    {
        ...
        if ($request->get('money') < 1)
            ...
            return back()->with('msg_no', '充值金额不达标');

        $mod = Recharge::findOrFail($id);
        if($mod->status == "2")return redirect(route('recharge.index'))->with('msg_no', 'Already approved.');
        $data = $request->all();
        try{
...
//                        'after_money' => $mod->member->money + $data['money'],
            ...
            DB::rollback();
            return respF('创建失败');
            return back()->with('msg_no', '创建失败');
        }

        // return responseSuccess('确认汇款成功', '', route('recharge.index'));
        return redirect(route('recharge.index'))->with("msg_ok", '确认汇款成功');
    }
